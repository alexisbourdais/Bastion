...
def main(main_file, mode, ckcompleteness, ckcontamination, gunccss, guncrrs, physetercontamination, krakencontamination, bucompleteness, budups, ck2completeness, ck2contamination, numcontigs):
    ...
    if (mode == 'table'):
        ...
        #Open checkm results
        CKfile = open('Checkm1_report.txt')
        for line in CKfile:
            record = line.replace("\n", "")
            if ('#' in record):
                continue
        ...
        #Physeter results
        PYfile = open('physeter_multi.report')
        for line in PYfile:
            record = line.replace("\n", "")
            split_list = record.split("\t")
            Genome = split_list[0]
            Genome = Genome.replace("-kraken", "")
            placement = split_list[1]
            conta = split_list[3]
        ...
        #Kraken results
        KAfile = open('kraken2_multi.report')
        for line in KAfile:
            record = line.replace("\n", "")
            split_list = record.split("\t")
            Genome = split_list[0]
            Genome = Genome.replace(".report", "")
            placement = split_list[1]
            conta = split_list[3]
        ...
        #Quast results
        QUfile = open('quast_multi.report')
        for line in QUfile:
            record = line.replace("\n", "")
